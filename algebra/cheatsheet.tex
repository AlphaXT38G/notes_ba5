...
\cheattitle{Algebra}{}{BA5}

\section{Integers}
\subsection{\texorpdfstring{IP $\Leftrightarrow$ SIP $\Leftrightarrow$ WOP}{IP <=> SIP <=> WOP}}

\begin{prop}[title=Induction Principle]
Let $S\subset\N$ such that
\begin{enumerate}
  \item $0\in S$;
...
\begin{prop}[title=Strong Induction Principle]
Let $S\subset\N$ such that
\begin{enumerate}
  \item $0\in S$;
  \item \textbf{if} $\bm{\{0,1,\ldots,n\}\subset S}$\textbf{,} then $n+1\in S$.
\end{enumerate}
Then $S=\N$.
\end{prop}

\begin{prop}[title=Well-Ordering Principle]
Every nonempty subset of $\N$ has a least element.
\end{prop}

\begin{thm}[title=Bézout]
$a,b\in\Z_+$, $d=\gcd(a,b)$. Run Euclid backwards:
\begin{itemize}
  \item $\exists\,x,y\in\Z$: $ax+by=d$.
  \item $ax+by=c$ solvable in $\Z$ $\iff$ $d\mid c$.
  \item $a,b$ coprime ($d=1$) $\iff$ $\exists\,x,y$: $ax+by=1$.
\end{itemize}
\end{thm}

\begin{defn}[title=Euler's totient $\varphi(n)$]
